...
class EXPORT_OT_dff(bpy.types.Operator, ExportHelper):
    ...
    bl_label       = "Dragon Ball OnLine DFF (.dff)"
    filename_ext   = ".dff"
    ...
    export_version  : bpy.props.EnumProperty(
        items =
        (
            ('0x33002', "DBO (v3.3.0.2)", "DBO PC (v3.3.0.2)"),
            ('0x34003', "DBO (v3.4.0.3)", "DBO PC (v3.4.0.3)"),
            ('0x36003', "DBO (v3.6.0.3)", "DBO PC (v3.6.0.3)"),
    ...
    def execute(self, context):
        ...
        context.scene['Dragon Ball OnLine_imported_version'] = self.export_version
        context.scene['Dragon Ball OnLine_custom_version']   = self.custom_version
            
        return {'FINISHED'}

    #######################################################
    def invoke(self, context, event):
        if 'Dragon Ball OnLine_imported_version' in context.scene:
            self.export_version = context.scene['Dragon Ball OnLine_imported_version']
        if 'Dragon Ball OnLine_custom_version' in context.scene:
            self.custom_version = context.scene['Dragon Ball OnLine_custom_version']
        ...
        for file in [os.path.join(self.directory,file.name) for file in self.files] if self.files else [self.filepath]:
            if file.endswith(".col"):
                ...
                if version in ['0x33002', '0x34003', '0x36003']:
                    context.scene['Dragon Ball OnLine_imported_version'] = version
                else:
                    context.scene['Dragon Ball OnLine_imported_version'] = "custom"
                    context.scene['Dragon Ball OnLine_custom_version'] = "{}.{}.{}.{}".format(
